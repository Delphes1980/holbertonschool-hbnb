...
def create_app(config_class="config.DevelopmentConfig"):
    ...
    with app.app_context():
        ...
        if admin_email is not None and admin_password is not None and User.query.count() == 0:
            admin_user = User(
                first_name='Admin',
                last_name='User',
                email=admin_email,
        ...
        if regular_user_email is not None and regular_user_password is not None and not User.query.filter_by(email=regular_user_email).first():
            regular_user = User(
                first_name='Regular',
                last_name='User',
